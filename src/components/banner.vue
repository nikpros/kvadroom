    ...
    .banner
        .banner-nav
            ...
            .nav-menu
                ul.nav-menu-items
                    li.menu-item
                        a(href="" @click.prevent="selectItem") отделка
                    li.menu-item
                        a(href="" @click.prevent="selectItem") архитектура
                    li.menu-item
                        a(href="" @click.prevent="selectItem") планировки
        .banner-descr
            .text
...
export default {
    ...
    mounted() {
        let logo = $('.nav-logo-brand path'),
            navItems = $('.menu-item a'),
            random = Math.random() * 360;

        $(logo).css('fill', `hsl(${random}, 100%, 40%)`);
        $(navItems).css('color', `hsl(${random}, 100%, 40%)`);
    },
    computed: {
        getCurrentImage() {
    ...
    methods: {
        ...
        selectItem(event) {
            ...
            this.currentSlide = $('.menu-item').index($(event.target).parent())
            $(logo).css('fill', `hsl(${random}, 100%, 40%)`);
            $(navItems).css('color', `hsl(${random}, 100%, 40%)`);
        }
    }
